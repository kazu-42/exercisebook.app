...
function createPendingJsonResponse(getRequestSignal: () => AbortSignal | undefined): {
  ...
  const response = Response.json(createDailyPlanPreviewResponseFixture(8));
    ...
    () =>
      new Promise<unknown>((_resolve, reject) => {
        ...
        if (requestSignal === undefined) {
          reject(new Error("The fetch signal was not captured."));
          return;
        }
  ...
  return { bodyReadStarted, response };
}
...
describe("daily plan preview loader", () => {
  ...
  it("preserves caller abort while a successful response body is pending", async () => {
    ...
    const callerAbort = new DOMException("Preview replaced.", "AbortError");
    let requestSignal: AbortSignal | undefined;
    const { bodyReadStarted, response } = createPendingJsonResponse(
      () => requestSignal,
    );
    vi.stubGlobal(
      "fetch",
